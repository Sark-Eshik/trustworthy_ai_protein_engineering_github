# src/figures/integrated.py
"""Integrated Reliability Figures Module (Figure F4 / P3).

Plots Antisymmetry Error vs. Epistasis Error to establish the causal mechanistic link
across single mutations. Outputs both PNG and PDF.
"""
...
class IntegratedFigureGenerator:
    """Generates Figure F4: Antisymmetry Error vs Epistasis Error."""
    ...
    def generate_figure(self, results_path: str = None, output_dir: str = None) -> Tuple[str, str]:
        ...
        res_path = results_path or os.path.join(data_dir, "final/reliability/integrated_reliability_analysis.parquet")
        out_dir = output_dir or self.config.paths.figures_dir
        os.makedirs(out_dir, exist_ok=True)

        if not os.path.exists(res_path):
            raise FileNotFoundError(f"Input file not found for integrated figure: {res_path}")

        df = pd.read_parquet(res_path)
        self.logger.info(f"Loaded {len(df)} records for Figure F4 generation.")

        x = df["antisymmetry_error"].values
        ...
        ax.scatter(
            ...
            alpha=0.75, 
            linewidth=0.8, 
            label="Mutation-level Error Aggregation"
        )
        ...
        x_grid = np.linspace(0.0, max(x), 100)
        ...
        ax.set_title("Figure F4: Mechanistic Error Coupling (Validation vs. Discovery)", fontsize=13, fontweight="bold", pad=15)
        ax.set_xlabel("Thermodynamic Antisymmetry Error (kcal/mol)", fontsize=11, labelpad=10)
        ax.set_ylabel("Epistasis Prediction Error |Pred_ab - Exp_ab| (kcal/mol)", fontsize=11, labelpad=10)
        ax.set_xlim(-0.05, max(x) + 0.1)
        ...
        png_path = os.path.join(out_dir, "F4_antisymmetry_vs_epistasis_error.png")
        pdf_path = os.path.join(out_dir, "F4_antisymmetry_vs_epistasis_error.pdf")
        
        plt.savefig(png_path, dpi=300)
        plt.savefig(pdf_path, format="pdf")
        plt.close()

        self.logger.info(f"Figure F4 successfully exported to: {png_path} and {pdf_path}")
        return png_path, pdf_path

def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Figure F4: Antisymmetry Error vs Epistasis Error.")
    parser.add_argument("--config", type=str, default="configs", help="Path to config directory.")
    parser.add_argument("--input", type=str, default=None, help="Path to integrated reliability analysis parquet.")
    parser.add_argument("--output", type=str, default=None, help="Directory to save generated figures.")
    args = parser.parse_args()

    try:
        generator = IntegratedFigureGenerator(config_path=args.config)
        generator.generate_figure(results_path=args.input, output_dir=args.output)
        print("SUCCESS")
    except Exception as e:
        print(f"ERROR: Integrated Figure generation failed: {e}", file=sys.stderr)
        sys.exit(1)
