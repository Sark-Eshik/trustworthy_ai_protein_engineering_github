# src/experiment2/statistics.py
"""Module for computing Experiment 2 statistics.

Performs Pearson and Spearman correlations, linear regression, and quantile comparisons
between Combined Sparsity and Epistasis Error. Saves the statistical summaries to CSV files.
"""
...
def compute_statistics(config_path: str = "configs", output_dir_override: str = None) -> tuple[str, str, str]:
    ...
    logger = get_logger(
        name="experiment2_statistics",
        log_dir=loader.config.paths.logs_dir,
        level=loader.config.logging.level,
    )
    
    logger.info("Initializing Experiment 2 statistical calculations...")
    print("Computing Experiment 2 statistics...")

    # Establish directories
    data_dir = loader.config.paths.data_dir
    results_dir = output_dir_override or os.path.join(loader.config.paths.results_dir, "experiment2")
    ...
    corr_csv_path = os.path.join(results_dir, "epistasis_correlations.csv")
    ...
    reg_csv_path = os.path.join(results_dir, "epistasis_regression.csv")
    ...
    avg_err_low = lowest_10_df["epistasis_error"].mean()
    avg_err_high = highest_10_df["epistasis_error"].mean()
    
    quantile_records = [
        {"subset": "Lowest 10% Sparsity (Dense)", "count": k, "mean_combined_sparsity": lowest_10_df["combined_sparsity"].mean(), "mean_epistasis_error": avg_err_low},
        {"subset": "Highest 10% Sparsity (Sparse)", "count": k, "mean_combined_sparsity": highest_10_df["combined_sparsity"].mean(), "mean_epistasis_error": avg_err_high},
        {"subset": "Difference (Sparse - Dense)", "count": k, "mean_combined_sparsity": highest_10_df["combined_sparsity"].mean() - lowest_10_df["combined_sparsity"].mean(), "mean_epistasis_error": avg_err_high - avg_err_low}
    ]
    df_quantile = pd.DataFrame(quantile_records)
    quantile_csv_path = os.path.join(results_dir, "epistasis_quantiles.csv")
    df_quantile.to_csv(quantile_csv_path, index=False)
    logger.info(f"Wrote quantile analysis to {quantile_csv_path}")

    print("\n--- Experiment 2 Statistical Summary ---")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Experiment 2 statistics.")
